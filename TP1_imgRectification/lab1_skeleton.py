...
affine_pts = H_E @ H @ np.transpose(pts_homo) 
affine_pts = np.transpose(affine_pts) # TODO
...
C1 = np.array([
	[aff_hor_1[0]*aff_ver_1[0] , aff_hor_1[0]*aff_ver_1[1] + aff_hor_1[1]*aff_ver_1[0] , aff_hor_1[1]*aff_ver_1[1]]
])
...
sigma,W= np.linalg.eig(mat_S)
K = W@np.sqrt(np.diag(sigma))
K = np.linalg.inv(K)
